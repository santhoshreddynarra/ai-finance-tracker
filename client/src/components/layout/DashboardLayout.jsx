...
const DashboardLayout = ({ children }) => {
  ...
  return (
    <div className="flex h-screen bg-[#0a0a14] overflow-hidden text-slate-300">
      {/* Desktop Sidebar */}
      <aside className="w-64 flex-shrink-0 bg-[#0a0a14] border-r border-white/5 hidden md:flex flex-col relative z-20">
        <SidebarContent />
      </aside>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          <aside className="w-64 bg-[#0a0a14] border-r border-white/5 flex flex-col relative z-50 h-full">
            <SidebarContent />
          </aside>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        {/* Navbar */}
        <header className="h-16 flex-shrink-0 bg-white/[0.01] backdrop-blur-md border-b border-white/5 flex items-center justify-between px-4 sm:px-6 z-10">
